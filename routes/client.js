...
router.post('/', async (request, response) => {
    const data = new Client({...request.body});
...
router.get('/:id', async (request, response) => {
    try {
        const data = await Client.findById(request.params.id);
        response.json(data);
    } catch (error) {
        response.status(500).json({ message: error.message });
    }
...
router.patch('/:id', async (request, response) => {
    try {
        ...
        const result = await Client.findByIdAndUpdate(id, updatedData, options);
